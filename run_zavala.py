...
for i, key in enumerate(keys):
    r = int(random.randint(key, shape=(), minval=0, maxval=1_000_000))    #uncommented for s10_real_mix
    # instances.append(generate_instance(key, num_scenarios=500, num_g=10, num_d=10, r=seeds[i]))   # for s10_real_mix
    instances.append(generate_instance(key, num_scenarios=500, num_g=10, num_d=10, r=r))
    instance_seeds.append(r)
...
for i in range(len(instances)):
    try:
        ...
        print(f"[SKIP] Instance {i} and seed {instance_seeds[i]}: solver failed with error: {e}")
        continue
